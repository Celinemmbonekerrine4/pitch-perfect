...
def setup_config(db_url, options, file_config, follower_ident):
    if follower_ident:
        db_url = _follower_url_from_home(db_url, follower_ident)
    db_opts = {}
...
@register.init
def _follower_url_from_home(url, ident):
...
@_follower_url_from_home.for_db("sqlite")
def _sqlite_follower_url_from_home(url, ident):
    url = sa_url.make_url(url)
...
@_follower_url_from_home.for_db("oracle")
def _oracle_follower_url_from_home(url, ident):
    url = sa_url.make_url(url)
    url.username = ident
    url.password = "xe"
    return url
