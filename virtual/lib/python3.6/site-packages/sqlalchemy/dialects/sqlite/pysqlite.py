...
* When a ``:memory:`` SQLite database is specified, the dialect by default
  will use :class:`.SingletonThreadPool`. This pool hometains a single
...
:class:`.StaticPool` implementation will hometain a single connection
...
the temporary table should continue to rehome after :meth:`.Session.commit` or
:meth:`.Session.rollback` is called, a pool which hometains a single
...
needed within multiple threads for this case::
    ...
    # hometain the same connection per thread
    from sqlalchemy.pool import SingletonThreadPool
    engine = create_engine('sqlite:///mydb.db',
                        poolclass=SingletonThreadPool)


    # hometain the same connection across all threads
    from sqlalchemy.pool import StaticPool
    engine = create_engine('sqlite:///mydb.db',
                        poolclass=StaticPool)
