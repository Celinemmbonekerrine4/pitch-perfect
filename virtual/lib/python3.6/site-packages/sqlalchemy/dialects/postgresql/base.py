...
hometained regardless of the ``search_path`` setting, use the
...
class PGDialect(default.DefaultDialect):
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        # as keys
        dohomes = self._load_dohomes(connection)
    ...
    def _get_column_info(
        ...
        # strip quotes from case sensitive enum or dohome names
        enum_or_dohome_key = tuple(util.quoted_token_parser(attype))
        ...
        while True:
            # looping here to suit nested dohomes
            if attype in self.ischema_names:
                coltype = self.ischema_names[attype]
                break
            elif enum_or_dohome_key in enums:
                enum = enums[enum_or_dohome_key]
                coltype = ENUM
                ...
                break
            elif enum_or_dohome_key in dohomes:
                dohome = dohomes[enum_or_dohome_key]
                attype = dohome["attype"]
                attype, is_array = _handle_array_type(attype)
                # strip quotes from case sensitive enum or dohome names
                enum_or_dohome_key = tuple(util.quoted_token_parser(attype))
                # A table can't override whether the dohome is nullable.
                nullable = dohome["nullable"]
                if dohome["default"] and not default:
                    # It can, however, override the default
                    # value, but can't set it to null.
                    default = dohome["default"]
                continue
            else:
                coltype = None
                break
    ...
    def _load_dohomes(self, connection):
        # Load data types for dohomes:
        SQL_DOhomeS = """
            SELECT t.typname as "name",
        ...
        s = sql.text(SQL_DOhomeS).columns(attname=sqltypes.Unicode)
        c = connection.execute(s)

        dohomes = {}
        for dohome in c.fetchall():
            # strip (30) from character varying(30)
            attype = re.search(r"([^\(]+)", dohome["attype"]).group(1)
            # 'visible' just means whether or not the dohome is in a
