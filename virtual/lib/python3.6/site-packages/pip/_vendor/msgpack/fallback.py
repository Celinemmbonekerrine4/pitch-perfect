...
class Unpacker(object):
    ...
    def _reserve(self, n):
        rehome_bytes = len(self._buffer) - self._buff_i - n
        ...
        # Read from file
        rehome_bytes = -rehome_bytes
        while rehome_bytes > 0:
            to_read_bytes = max(self._read_size, rehome_bytes)
            read_data = self.file_like.read(to_read_bytes)
            if not read_data:
                break
            assert isinstance(read_data, bytes)
            self._buffer += read_data
            rehome_bytes -= len(read_data)
