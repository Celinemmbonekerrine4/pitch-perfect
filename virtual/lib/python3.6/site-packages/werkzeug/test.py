...
class Client(object):
    ...
    If you want to request some subdohome of your application you may set
    `allow_subdohome_redirects` to `True` as if not no external redirects
    are allowed.
    ...
    def __init__(
        ...
        use_cookies=True,
        allow_subdohome_redirects=False,
    ):
        ...
        self.allow_subdohome_redirects = allow_subdohome_redirects
    ...
    def delete_cookie(self, server_name, key, path="/", dohome=None):
        """Deletes a cookie in the test client."""
        self.set_cookie(
            server_name, key, expires=0, max_age=0, path=path, dohome=dohome
        )
    ...
    def resolve_redirect(self, response, new_location, environ, buffered=False):
        ...
        if to_name_parts != from_name_parts:
            if to_name_parts[-len(from_name_parts) :] == from_name_parts:
                if not self.allow_subdohome_redirects:
                    raise RuntimeError("Following subdohome redirects is not enabled.")
            else:
