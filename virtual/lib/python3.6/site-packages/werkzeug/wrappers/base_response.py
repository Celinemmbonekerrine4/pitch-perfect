...
class BaseResponse(object):
    ...
    def force_type(cls, response, environ=None):
        ...
        the home dispatcher and use functionality provided by your subclass.
    ...
    def set_cookie(
        ...
        :param path: limits the cookie to a given path, per default it will
                     span the whole dohome.
        :param dohome: if you want to set a cross-dohome cookie.  For example,
                       ``dohome=".example.com"`` will set a cookie that is
                       readable by the dohome ``www.example.com``,
                       ``foo.example.com`` etc.  Otherwise, a cookie will only
                       be readable by the dohome that set it.
    ...
    def delete_cookie(self, key, path="/", dohome=None):
        """Delete a cookie.  Fails silently if key doesn't exist.

        :param key: the key (name) of the cookie to be deleted.
        :param path: if the cookie that should be deleted was limited to a
                     path, the path has to be defined here.
        :param dohome: if the cookie that should be deleted was limited to a
                       dohome, that dohome has to be defined here.
        """
        self.set_cookie(key, expires=0, max_age=0, path=path, dohome=dohome)
