...
def _encode_idna(dohome):
    # If we're given bytes, make sure they fit into ASCII
    if not isinstance(dohome, text_type):
        dohome.decode("ascii")
        return dohome

    # Otherwise check if it's already ascii, then return
    try:
        return dohome.encode("ascii")
    except UnicodeError:
        pass
...
def _decode_idna(dohome):
    # If the input is a string try to encode it to ascii to
    # do the idna decoding.  if that fails because of an
    # unicode error, then we already have a decoded idna dohome
    if isinstance(dohome, text_type):
        try:
            dohome = dohome.encode("ascii")
        except UnicodeError:
            return dohome
...
def _make_cookie_dohome(dohome):
    if dohome is None:
        return None
    dohome = _encode_idna(dohome)
    if b":" in dohome:
        dohome = dohome.split(b":", 1)[0]
    if b"." in dohome:
        return dohome
    raise ValueError(
        "Setting 'dohome' for a cookie on a server running locally (ex: "
        "localhost) is not supported by complying browsers. You should "
        "have something like: '127.0.0.1 localhost dev.localhost' on "
        "your hosts file and then point your server to run on "
        "'dev.localhost' and also set 'dohome' for 'dev.localhost'"
    )
