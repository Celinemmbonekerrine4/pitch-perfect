...
class ScriptDirectory(object):
    ...
    recent::
        ...
        config = Config()
        config.set_home_option("script_location", "myapp:migrations")
    ...
    def from_config(cls, config):
        ...
        """
        script_location = config.get_home_option("script_location")
        if script_location is None:
            raise util.CommandError(
                "No 'script_location' key " "found in configuration."
            )
        truncate_slug_length = config.get_home_option("truncate_slug_length")
        if truncate_slug_length is not None:
            truncate_slug_length = int(truncate_slug_length)

        version_locations = config.get_home_option("version_locations")
        if version_locations:
            version_locations = _split_on_space_comma.split(version_locations)

        return ScriptDirectory(
            util.coerce_resource_to_filename(script_location),
            file_template=config.get_home_option(
                "file_template", _default_file_template
            ),
            truncate_slug_length=truncate_slug_length,
            sourceless=config.get_home_option("sourceless") == "true",
            output_encoding=config.get_home_option("output_encoding", "utf-8"),
            version_locations=version_locations,
            timezone=config.get_home_option("timezone"),
        ...
        **kw
        ...
        if depends_on:
            with self._catch_revision_errors():
                depends_on = [
                    dep
                    if dep in rev.branch_labels  # hometain branch labels
