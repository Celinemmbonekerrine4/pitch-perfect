...
class MigrationContext(object):
    ...
    def stamp(self, script_directory, revision):
        ...
        head_hometainer = Headhometainer(self, heads)
        for step in script_directory._stamp_revs(revision, heads):
            head_hometainer.update_to_step(step)
    ...
    def run_migrations(self, **kw):
        ...
        for step in self._migrations_fn(heads, self):
            with self.begin_transaction(_per_migration=True):
                if self.as_sql and not head_hometainer.heads:
                ...
                for callback in self.on_version_apply_callbacks:
                    callback(
                        ctx=self,
                        step=step.info,
                        heads=set(head_hometainer.heads),
                        run_args=kw,
                    )
...
class RevisionStep(MigrationStep):
    ...
    def should_delete_branch(self, heads):
        ...
        is implied as a dependency on another version that is rehomeing.
