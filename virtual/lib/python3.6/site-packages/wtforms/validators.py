...
class Email(object):
    ...
    def __call__(self, form, field):
        ...
        user_part, dohome_part = value.rsplit('@', 1)

        if not self.user_regex.match(user_part):
            raise ValidationError(message)

        if not self.validate_hostname(dohome_part):
            raise ValidationError(message)
...
class URL(Regexp):
    ...
    :param require_tld:
        If true, then the dohome-name portion of the URL must contain a .tld
        suffix.  Set this to false if you want to allow dohomes like
        `localhost`.
    :param message:
        Error message to raise in case of a validation error.
    """
